...
- Result: 관리자와 환자가 동일한 대기 순서를 확인할 수 있도록 운영 데이터 흐름을 개선했습니다.

## Key Numbers

- 4 data domains: 환자, 예약, 대기열, 공지사항
- 5 public screen flows: 환자 홈, 예약 캘린더, 예약 신청, 실시간 대기, 진료 완료
- 4-month team project: 2026.03 ~ 2026.06
...
- 담당: Supabase(PostgreSQL) 기반 데이터 구조 설계 및 Flutter 연동
- 담당: 환자·예약·대기열·공지사항 데이터 조회/저장 로직 구현
- 담당: 관리자 웹 대기 관리 및 공지 관리 기능 개발 지원
- 담당: 상태 변경 후 최신 목록 재조회와 API 오류 예외 처리
